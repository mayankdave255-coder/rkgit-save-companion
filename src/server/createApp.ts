...
/**
 * Minimal shape of the `@google/genai` client that this app actually uses.
 * Declaring it explicitly (rather than importing GoogleGenAI's own type)
 * lets tests inject a lightweight fake/mock without needing network access
 * or a real API key.
 */
...
export interface CreateAppOptions {
  /** Factory used to build the Gemini client. Overridable for tests. */
  geminiClientFactory?: () => GeminiClient;
  /** Overrides process.env.GEMINI_API_KEY (useful in tests). */
  apiKey?: string;
}
...
export const SYSTEM_INSTRUCTION = `You are the chief Emergency Medical Triage Specialist and Campus Safety Officer for Raj Kumar Goel Institute of Technology (RKGIT), located in Ghaziabad, Uttar Pradesh, India.
...
/** Structured fallback response used when no Gemini API key is configured. */
export function buildOfflineFallbackTriage(text?: string) {
  return {
    severity: 'MODERATE',
...
/**
 * Parses a raw or data-URL-encoded base64 image string into the
 * `{ mimeType, data }` shape Gemini's `inlineData` part expects.
 */
export function parseImagePayload(image: string): { mimeType: string; data: string } {
...
/**
 * Builds the Express application containing all RKGIT Safe Companion API
 * routes. Kept separate from `server.ts` (which wires up Vite/static
 * serving and `app.listen`) so it can be exercised in tests via supertest
 * without hitting the network or starting a real HTTP server.
 */
export function createApp(options: CreateAppOptions = {}): Express {
  ...
  function hasApiKey(): boolean {
    return !!options.geminiClientFactory || !!(options.apiKey ?? process.env.GEMINI_API_KEY);
  }

  function getGeminiClient(): GeminiClient {
    if (!geminiClient) {
      if (options.geminiClientFactory) {
        geminiClient = options.geminiClientFactory();
      } else {
  ...
  app.post('/api/triage', async (req, res) => {
    try {
      ...
      if (image && typeof image === 'string') {
        parts.push({ inlineData: parseImagePayload(image) });
      }
