...
export const OfflineGuidesSection: React.FC<OfflineGuidesSectionProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 sm:p-5 rounded-3xl bg-[#0a1524]/85 border-2 border-cyan-500/20">
        <div>
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full bg-[#060c16] border-2 border-cyan-500/20 rounded-2xl pl-10 pr-3.5 py-2 text-xs font-medium text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-cyan-400 transition"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredGuides.map((guide) => (
          <div
            ...
            className="p-5 rounded-3xl bg-[#0a1524]/85 border-2 border-cyan-500/20 hover:border-cyan-400/70 hover:shadow-[0_0_20px_-6px_rgba(34,211,238,0.5)] hover:-translate-y-1 transition-all flex flex-col justify-between gap-3.5 cursor-pointer group"
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-3">
                <div className="w-10 h-10 rounded-2xl bg-[#060c16] border-2 border-cyan-500/20 flex items-center justify-center group-hover:border-cyan-400/60 transition">
                  {getGuideIcon(guide.iconName)}
                </div>
                <span className="px-2.5 py-1 rounded-xl text-[10px] font-black bg-[#060c16] text-slate-300 border-2 border-cyan-500/20">
                  {language === 'hi' ? guide.categoryHi : guide.category}
                </span>
              </div>
              <h3 className="text-sm font-extrabold text-slate-100 group-hover:text-cyan-300 transition leading-snug">
            ...
            <div className="pt-2.5 border-t-2 border-cyan-500/20 flex items-center justify-between text-xs text-cyan-400 font-extrabold">
          ...
          <div className="relative w-full max-w-2xl rounded-3xl bg-[#0a1524]/85 border-2 border-cyan-500/25 overflow-hidden my-auto max-h-[90vh] flex flex-col">
            ...
            <div className="p-4 sm:p-5 bg-gradient-to-r from-red-950/80 via-slate-900 to-slate-900 border-b-2 border-cyan-500/20 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-2xl bg-[#060c16] border-2 border-cyan-500/20 flex items-center justify-center flex-shrink-0">
                  {getGuideIcon(selectedGuide.iconName)}
                </div>
                <div>
                  <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-black bg-red-950 text-red-400 border border-red-800">
              ...
              <button
                ...
                className="p-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border border-cyan-500/25 cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-4 sm:p-6 space-y-5 overflow-y-auto">
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <span className="text-[11px] text-slate-300 font-bold flex items-center gap-1.5 bg-[#060c16] px-2.5 py-1 rounded-xl border border-cyan-500/20">
              ...
              {/* Immediate Action */}
              <div className="p-4 rounded-2xl bg-red-950/40 border-2 border-red-800/80 space-y-1">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-3">
                  {selectedGuide.steps.map((step) => (
                    <div
                      key={step.stepNumber}
                      className="p-4 rounded-2xl bg-[#060c16] border-2 border-cyan-500/20 flex items-start gap-3.5"
                    >
                      <span className="flex-shrink-0 w-7 h-7 rounded-xl bg-red-950 border-2 border-red-800 text-red-400 flex items-center justify-center text-xs font-black">
                        {step.stepNumber}
                      </span>
                      <div>
              ...
              {/* Campus Advice */}
              <div className="p-4 rounded-2xl bg-[#060c16] border-2 border-cyan-500/20 space-y-1">
            ...
            {/* Footer */}
            <div className="p-4 bg-[#060c16] border-t-2 border-cyan-500/20 flex items-center justify-between">
              <button
                onClick={onOpenQuickDial}
                className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 active:translate-x-0.5 active:translate-y-0.5 text-white text-xs font-bold border-2 border-red-400 transition cursor-pointer"
              >
              ...
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-800 text-slate-200 text-xs font-bold border-2 border-cyan-500/25 hover:bg-slate-700 cursor-pointer"
              >
