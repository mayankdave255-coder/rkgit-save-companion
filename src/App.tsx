...
export default function App() {
  ...
  // Offline matching heuristic
  const findOfflineMatch = (text: string, location: string): TriageResult => {
    const q = text.toLowerCase();
    let guide = OFFLINE_FIRST_AID_GUIDES[0]; // default CPR

    if (q.includes('burn') || q.includes('acid') || q.includes('chemical') || q.includes('जल') || q.includes('आग')) {
    ...
    } else if (q.includes('shock') || q.includes('electric') || q.includes('current') || q.includes('करंट') || q.includes('बिजली')) {
